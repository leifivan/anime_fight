 ...
 *
 * @param Menu Enabled
 * @desc Toggles whether the menu is enabled during battle.
 * @type boolean
 * @default true
 *
 * @help
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_LMBS');
    ...
    Moghunter.lmbsMenuEnabled = Moghunter.parameters['Menu Enabled'] === "true";
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
    ...
    //edit
	if (command === "lmbs_set_skill_2")  {
		var actorID = Number(args[1]);
		var slotID = Number(args[3]);
		var slillID	 = Number(args[5]);
		$gameParty.members().forEach(function(actor) {
			if(actor._actorId === actorID) {
				actor.lmbsSetSkill_2(slotID,slillID)
			};
		}, this);
	};
	return true;
...
Game_Battler.prototype.initMembers = function() {
	...
	//edit
	this._lmbs_skills_2 = [null,null,null,null,null,null,null];
...
// * Lmbs Set Skill 2
//==============================
Game_Battler.prototype.lmbsSetSkill_2 = function(value,skillid) {
	if (value > 6) {return};
    this._lmbs_skills_2[Math.abs(value)] = Math.abs(skillid);
};
...
Game_Battler.prototype.lmbsAttackAction = function() {
   ...
   // this.lmbsAction(this.attackSkillId(),0,null);
   	 ...
   	 var skillID = this.lmbsSetSkillIdActor_2();
...
// edit
//==============================
// * Lmbs Skill Action Actor 2
//==============================
Game_Battler.prototype.lmbsSetSkillIdActor_2 = function() {
     if (this.lmbsCanUseGroundSkill() && !this.lmbsCanFly()) {
		 if (Input.isPressed('left') || Input.isPressed('right')) {
			 return this._lmbs_skills_2[1];
		 } else if (Input.isPressed('down')){
			 return this._lmbs_skills_2[2];
		 } else {
		     return this._lmbs_skills_2[0];
		 };
	 } else if (this.lmbsCanUseAerialSkill()) {
		 if (Input.isPressed('left') || Input.isPressed('right')) {
			 return this._lmbs_skills_2[4];
		 } else if (Input.isPressed('down')){
			 return this._lmbs_skills_2[5];
		 } else if (Input.isPressed('up')){
			 return this._lmbs_skills_2[6];			 
		 } else {
		     return this._lmbs_skills_2[3];
		 };
	 };
	 return 0;
};
...
Game_Battler.prototype.lmbsMakeSkillList = function() {
	...
	if (this.isActor()){
        ...
        // edit leif 
		for (var i = 0; i < this._lmbs_skills_2.length; i++) {
			var skillId = this._lmbs_skills_2[i];
			if (skillId) {
				var skill = $dataSkills[skillId];
				if (skill) {this._lmbsAISkillList.push(skill)};
			};
...
BattleManager.lmbsCanCallMenu = function() {
	if (!Moghunter.lmbsMenuEnabled) {return false}; //EDITED Jan 6 2018
